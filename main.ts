...
function attemptJump () {
    ...
    if (hero.isHittingTile(CollisionDirection.Bottom)) {
        hero.vy = controller.lightLevel() * -0.05 * pixelsToMeters
    } else if (canDoubleJump) {
        doubleJumpSpeed = -3 * pixelsToMeters
        // Good double jump
        if (hero.vy >= -40) {
            doubleJumpSpeed = controller.lightLevel() * -0.05 * pixelsToMeters
...
game.onUpdate(function () {
    ...
    if (controller.lightLevel() >= 50) {
        light.setPixelColor(1, 0xff0000)
    } else {
        light.clear()
    }
    if (controller.lightLevel() >= 150) {
        light.setPixelColor(2, 0xff0000)
    } else {
        light.clear()
    }
    if (controller.lightLevel() >= 200) {
        light.setPixelColor(3, 0xff0000)
    } else {
        light.clear()
        if (controller.lightLevel() >= 240) {
